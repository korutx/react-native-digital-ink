...
//      self.label.text = @"Hello from Native View";
 ...
 */
- (void)recognizeInk:(RCTPromiseResolveBlock)resolve
                    rejecter:(RCTPromiseRejectBlock)reject  {
  if (self.strokes.count == 0) {
    reject(@"error", @"No ink to recognize", nil);
//    [self.delegate displayMessage:@"No ink to recognize"];
    return;
  }
  if (![self.modelManager isModelDownloaded:self.model]) {
    reject(@"error", @"Recognizer model not downloaded", nil);
//    [self.delegate displayMessage:@"Recognizer model not downloaded"];
  ...
  if (self.recognizer == nil) {
    ...
    if (self.recognizer == nil) {
      reject(@"error", @"Could not initialize recognizer", nil);
      ...
      recognizeInk:ink
        completion:^(MLKDigitalInkRecognitionResult *_Nullable result, NSError *_Nullable error) {
          ...
          if (strongSelf == nil) {
            ...
            reject(@"error", @"self == nil handling recognition completion", nil);
            return;
          }
...
//            recognizedInk.text = result.candidates[0].text;
            ...
            if (result.candidates[0].score != nil) {
              ...
              NSDictionary *data = @{
                @"score": @(result.candidates[0].score.floatValue),
...
//            [strongSelf.delegate displayMessage:@"Recognition error"];
            reject(@"error", @"Recognition error", nil);
          }
